...
// WiFi Credentials
#define WIFI_SSID "tb84-iot2"
#define WIFI_PASS "welcome2iot"
...
int d = 0;                 // DISPLAY mode: 0 means temperature bigger, 1 means humidity bigger, 2=clock
...
void showtemp(float temp, float humid, int info) {
  ...
  timeClient.update();
  clk = timeClient.getFormattedTime();
  ...
  if (info == 0) {
    for (x=0; x<=32; x++) {
      ...
      display.setTextColor(SSD1306_WHITE); // Draw white text
      display.setCursor(2, x+31);     // Start at top-left corner
      display.print(clk);
      display.display();
      delay(30);
    }
